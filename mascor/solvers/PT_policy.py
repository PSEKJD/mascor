...
from mascor.models import actor
from mascor.models import critic
import numpy as np
import re
import torch
...
class policy():
    def __init__(self, args, pt_epoch = None):
        ...
        if args.z_type == 'default':
            ...
            z_dim = 24
        elif args.z_type == 'fft':
            raise RuntimeError("Error: {} type z token is not existed".format(args.z_type))
        ...
        # actor-critic part loading
        save_path = os.path.join(DATASET_DIR, '{country}/{region}/oracle_dataset_{option}_sample_{sample}/checkpoint_transformer_des_{des}_z_{z}_z_type_{z_type}'.format(country = args.target_country, 
                                                                                                                                                         region = args.region,
                                                                                                                                                         option = args.design_option,
                                                                                                                                                         sample = args.sample_size,
                                                                                                                                                         des = args.des_token,
    ...
    def compute_actions(self, des, z, ctg, rtg, s, a, t, mask, mode = "mu"):
        ...
        if mode == "mu":
            ...
            action_dist = torch.distributions.Normal(loc = mu_action_preds[:,-1,:], scale = torch.exp(std_action_preds[:,-1,:]))
            action  = action_dist.sample().clamp(mu_action_preds[:,-1,:]-1.645*torch.exp(std_action_preds[:,-1,:]), 
    ...
    def compute_goals(self, des, z, s, a, co2, r, t, mask, limit, buffer, step, simcase = "uq"):
        if self.critic is None:
            raise RuntimeError("Error: critic model is not existed") 
        ...
        if simcase == "rbdo":
            if step == 1:
                ...
                limit = buffer.ctg_scaler.transform(np.array([[limit]])) # scaler input = 2d array
                pf = 1-ctg_dist.cdf(torch.tensor(limit).to(dtype = torch.float32, device = self.device))
                mu_co2_preds = mu_ctg_preds*buffer.ctg_std + buffer.ctg_mu
                std_ctg_preds = torch.exp(log_std_ctg_preds)
                std_co2_preds = std_ctg_preds*buffer.ctg_std
            
                # LCOX calculation
                # RTG --> Profit        
                std_rtg_preds = torch.exp(log_std_rtg_preds)
                mu_profit = mu_rtg_preds*buffer.rtg_std + buffer.rtg_mu
                std_profit = std_rtg_preds*buffer.rtg_std
                
                return (mu_profit, std_profit, pf, mu_co2_preds, std_co2_preds, 
                        mu_rtg_preds, mu_ctg_preds)
            else:
